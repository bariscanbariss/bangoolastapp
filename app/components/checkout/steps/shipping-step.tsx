...
const ShippingStep = ({ cart }: ShippingStepProps) => {
  ...
  const { data: shippingOptions, isLoading: isLoadingShippingOptions, error: queryError } =
    useQuery({
  ...
  if (queryError || !shippingOptions?.length) {
    return (
      <View className="flex-1 items-center justify-center py-8">
        <Text className="text-red-500 text-center px-4">
          {queryError?.message || l10n.getString('no-region-id')}
        </Text>
      </View>
    );
  }
